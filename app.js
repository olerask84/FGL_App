...
function renderScoreCard(player) {
  ...
  function focusSameHoleOnNextPlayer(currentPlayerId, holeIndex) {
    ...
    setTimeout(() => {
        const nextHoleInput = document.querySelector(
          `.panel input[data-player-id="${nextPlayer.id}"][data-hole="${holeIndex}"]`
        );
        if (nextHoleInput) {
          nextHoleInput.focus();
  ...
  function createHoleRow(holeNum, holeIndex) {
    ...
    checkboxWrap.style.gap = '0.3rem';
    
    // Vis kun label på første hul
    if (holeIndex === 0) {
      const checkLabel = document.createElement('span');
      checkLabel.textContent = 'Tættest pinnen';
      checkLabel.style.fontSize = '0.85rem';
      checkLabel.style.color = 'var(--muted)';
      checkLabel.style.marginRight = '0.3rem';
